...
func TestFlashExplicitEffortReachesPreparedUpstreamBody(t *testing.T) {
	for _, effort := range []string{"low", "high", "max", "invalid_probe_effort"} {
		for _, nested := range []bool{false, true} {
			...
			applyChatReasoning(obj, translate.ChatRequest{Model: "workbuddy/deepseek-v4.1-flash", ReasoningEffort: raw}, "high", providers.ModelCapabilities{ReasoningOptions: []string{"high"}, ReasoningDefault: "high"})
			body, _ := json.Marshal(obj)
			var sent map[string]any
			if err := json.Unmarshal(PrepareBody(body), &sent); err != nil {
				t.Fatal(err)
			}
			if sent["reasoning_effort"] != effort || sent["reasoning"] != nil {
				t.Fatalf("effort=%s nested=%v body=%s", effort, nested, PrepareBody(body))
			}
		}
	}
}

func TestFlashCatalogDefaultDoesNotLimitExplicitEfforts(t *testing.T) {
	...
	for _, stored := range []string{"", "low", "max"} {
		obj := map[string]any{}
		applyChatReasoning(obj, translate.ChatRequest{Model: entry.ID}, stored, caps)
		want := stored
		if want == "" {
			want = "high"
		}
		if obj["reasoning_effort"] != want {
			t.Fatalf("stored=%q body=%v", stored, obj)
		}
	}
